...
container_name="orbitfactory-fact22-postgres-proof"
database_name="orbitfactory_fact22_proof"
database_user="orbitfactory"
database_password="fact22-local-proof"
...
cleanup() {
  ...
  if [[ "$created_container" == "true" ]]; then
    ...
    docker container inspect "$container_name" >/dev/null 2>&1 && exit 1
  fi
  exit "$test_status"
}
trap cleanup EXIT
...
npx vitest run tests/control-plane.postgres.test.ts
