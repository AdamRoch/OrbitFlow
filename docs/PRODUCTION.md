...
The deployment source is [AdamRoch/OrbitFlow](https://github.com/AdamRoch/OrbitFlow), branch `main`, repository root, using the existing `Dockerfile` and `railway.json`. Automatic deployments and Railway's Wait for CI are enabled. GitHub Actions runs only dependency installation, typecheck, and build; testing stays local. Run one replica with the standard ten restarts on failure. For each release, verify the GitHub commit SHA in the successful Railway deployment and check `/healthz` plus the affected UI. See [ADR 0004](../ADR/0004-github-deployment-source.md).

Leave Railway's deployment healthcheck gate unset. The new executor cannot start while the previous process holds the PostgreSQL advisory lock; keeping the old process alive until the new one passes its healthcheck prevents the handoff. The normal rollout stops the old process and lets the new process restart and acquire the lock. This single-executor deployment has a brief interruption; it is not a zero-downtime rollout. Check for active runs before merging a release, and verify HTTP health afterward. Never remove the executor lock to allow overlapping runs.
