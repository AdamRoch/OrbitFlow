...
OpenClaw `2026.4.15` is pinned. Completion accepts only that release's gateway response shape: the outer `runId` must equal the request's deterministic idempotency key, `status` must be `ok`, `summary` must be `completed`, and metadata must report `aborted: false`, `livenessState: "working"`, and matching `stop` reasons. A `replayInvalid` turn without a submission gets one corrective wake that preserves prior work and permits only `submit_result`. A second miss fails through the confirmed-failure path. Usage comes only from `result.meta.agentMeta.usage`, which represents the complete session stream; `lastCallUsage` is never substituted.
