...
Configure a **new** Telegram bot using `TELEGRAM_BOT_TOKEN` and your private chat's `TELEGRAM_CHAT_ID`. Do not reuse the original OrbitFlow bot. Restart the app and enable Telegram on exactly one agent. `/start` opens buttons for **Workflows**, **Agents**, and **Runs**; the same commands appear in Telegram's command menu. Workflow details show assigned agents, models, limits, and approval rules. Choose **Start workflow**, then send the request. `/improve` lets you choose an improvement workflow and an approved source application. Browsing makes no model calls and does not change configurations. Agent details identify the default chat recipient; browsing another agent does not switch it.

Plain messages reach the connected agent with its saved instructions, skills and memory, unless the bot is waiting for a workflow request or revision feedback. Pending requests are named in replies; `/cancel` clears the pending request without stopping existing runs. Run cards show status and usage, link to the exact run in the public studio, and offer approval, change requests, and a confirmed **Stop run** action. `/runs` lists only runs started in this chat, with active runs first; source selection can use approved applications from the operator's workspace. Buttons and pending requests expire after 24 hours. Changed workflow settings and stale approval buttons require a fresh selection. Other chats cannot operate these buttons. Credentials remain server-side.

Advanced commands remain available: `/build <workflow ID> <request>`, `/improve <workflow ID> <approved run ID> <request>`, `/status <run ID>`, `/approve <run ID> <approval token>`, and `/reject <run ID> <approval token> <feedback>`. `/improve <request>` still uses the latest approved application when exactly one improvement workflow exists. `/help` explains commands and routing. The web UI remains the place to edit agent and workflow configuration.
