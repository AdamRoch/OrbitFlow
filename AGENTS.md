...
- FACT-14's strict Software Factory E2E gate (real OpenClaw + OpenRouter, shipped-template prompt, bounded output-token budget) lives in `scripts/fact-14-e2e-proof.mjs`; prove it with `npm run fact14:proof`. FACT-34 extends the exact 0015 Factory Implementer prompt and exact seeded tester/graph contracts through `db/migrations/0022-factory-demo-contract.sql`; the documented demo task lives in `docs/software-factory-demo.md`.
...
- FACT-31's Compose `engine` runs the production PostgreSQL consumer, dispatcher, scheduler, and OpenClaw adapter; readiness requires successful consumer and dispatcher polls. Prove it with `npm run fact31:proof`.
