...
test(`FACT-12 upgrades the exact ${CURRENT_MAIN_SHA} migration history forward`, async () => {
  ...
  try {
    ...
    const fact12Migration = await migratePostgres({ databaseUrl, log: () => {} });
    assert.deepEqual(fact12Migration.applied, ["0010-coding-tool-usage.sql"]);
    ...
    assert.deepEqual(
      ...
      [...Object.keys(CURRENT_MAIN_MIGRATIONS), "0010-coding-tool-usage.sql"],
    );
