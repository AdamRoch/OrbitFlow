...
async function run(name, fn) {
  ...
  try {
    await fn();
    process.stderr.write(`ok ${passed + failed + 1} - ${name}\n`);
    passed++;
  } catch (err) {
    process.stderr.write(`not ok ${passed + failed + 1} - ${name}\n`);
...
const expectedColumns = {
  ...
  agents: ["id","name","role","system_prompt","model","coding_tool_enabled","guardrails","interaction_rules","channel_binding","memory","openclaw_ref","created_at","updated_at"],
  ...
  channel_intakes: ["run_id","workflow_id","provider","conversation_key","status","last_inbound_message_id","last_question","clarification_count","validated_spec","created_at","updated_at"],
  channel_completion_events: ["run_id","completion_message_id","final_outbound_message_id","created_at","reported_at"],
  ...
  cost_events: ["id","run_id","agent_id","model","tokens_in","tokens_out","computed_cost","created_at","updated_at","cache_read_tokens","cache_write_tokens"],
};

const expectedEnums = {
  message_type: ["output","feedback","question","answer","channel_inbound","channel_outbound","cron_tick","system"],
...
const requiredConstraints = [
  ...
  "cost_events_cache_write_tokens_nonnegative","cost_events_computed_cost_nonnegative",
  "cost_events_tokens_in_nonnegative","cost_events_tokens_out_nonnegative",
  "dependencies_blocked_ticket_fk","dependencies_blocker_ticket_fk","dependencies_edge_unique",
  "dependencies_not_self","messages_payload_object","messages_run_sequence_unique",
  ...
  "openclaw_dispatch_inputs_generation_positive","openclaw_dispatch_inputs_wake_input_object",
  "schedule_ticks_key_not_blank","schedule_ticks_schedule_key_unique",
  ...
  "workflow_fanout_groups_max_positive","workflow_fanout_groups_node_config_object",
  "workflow_fanout_groups_node_not_blank","workflow_fanout_members_pkey",
  "workflow_fanout_members_position_nonnegative","workflow_fanout_members_ticket_unique",
  "workflow_dispatches_activation_unique","workflow_dispatches_agent_model_not_blank",
  "workflow_dispatches_attempt_nonnegative","workflow_dispatches_generation_nonnegative",
  "workflow_dispatches_idempotency_key_key","workflow_dispatches_idempotency_not_blank",
  "workflow_dispatches_input_object","workflow_dispatches_node_not_blank",
...
try {
  ...
  assert.equal(identity.rows[0].name, process.env.ORBITFACTORY_FACT6_PROOF_DATABASE);

  await run("applies the complete chain from empty and then no-ops", async () => {
    const before = await client.query(`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_type = 'BASE TABLE'`);
    assert.deepEqual(before.rows, []);
    const first = await migrateQuietly({ databaseUrl });
    const committed = await committedMigrationFiles();
    assert.deepEqual(first.applied, committed);
    const second = await migrateQuietly({ databaseUrl });
    assert.deepEqual(second.applied, []);
  });

  await run("fails closed on migration history drift, lock contention, and failed SQL", async () => {
    const baseline = await client.query("SELECT version, checksum, applied_at FROM schema_migrations ORDER BY version");
    const baselineByVersion = new Map(baseline.rows.map((row) => [row.version, row]));

    await client.query("UPDATE schema_migrations SET checksum = $1 WHERE version = $2", ["0".repeat(64), "0001-control-plane.sql"]);
    await assert.rejects(() => migrateQuietly({ databaseUrl }), /does not match its committed checksum/);
    await client.query("UPDATE schema_migrations SET checksum = $1 WHERE version = $2", [baselineByVersion.get("0001-control-plane.sql").checksum, "0001-control-plane.sql"]);

    await client.query("INSERT INTO schema_migrations (version, checksum) VALUES ('9999-missing.sql', $1)", ["f".repeat(64)]);
    await assert.rejects(() => migrateQuietly({ databaseUrl }), /is missing from the committed migration directory/);
    await client.query("DELETE FROM schema_migrations WHERE version = '9999-missing.sql'");

    const removed = baselineByVersion.get("0002-tickets.sql");
    await client.query("DELETE FROM schema_migrations WHERE version = '0002-tickets.sql'");
    await assert.rejects(() => migrateQuietly({ databaseUrl }), /was introduced before already-applied 0003-message-plane.sql/);
    await client.query("INSERT INTO schema_migrations (version, checksum, applied_at) VALUES ($1, $2, $3)", [removed.version, removed.checksum, removed.applied_at]);
    ...
    try {
      await assert.rejects(
        () => migrateQuietly({ databaseUrl: failedDatabaseUrl.toString(), migrationDirectory: FAILED_MIGRATION_DIRECTORY }),
        /deliberately_missing_table/,
      );
      const failedClient = new Client({ connectionString: failedDatabaseUrl.toString() });
      await failedClient.connect();
      try {
        const journal = await failedClient.query("SELECT version FROM schema_migrations");
        assert.deepEqual(journal.rows, []);
        const rolledBack = await failedClient.query("SELECT to_regclass('public.must_roll_back_with_failed_migration') AS name");
    ...
    const restored = await client.query("SELECT version, checksum, applied_at FROM schema_migrations ORDER BY version");
    assert.deepEqual(restored.rows, baseline.rows);
  });

  await run("matches tables, columns, native types, constraints, keys, and indexes", async () => {
    const columns = await client.query(`SELECT table_name, array_agg(column_name::text ORDER BY ordinal_position) AS columns FROM information_schema.columns WHERE table_schema = 'public' GROUP BY table_name ORDER BY table_name`);
    assert.deepEqual(
    ...
    const enums = await client.query(`SELECT t.typname, array_agg(e.enumlabel::text ORDER BY e.enumsortorder) AS values FROM pg_type t JOIN pg_enum e ON e.enumtypid = t.oid JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'public' GROUP BY t.typname ORDER BY t.typname`);
    assert.deepEqual(Object.fromEntries(enums.rows.map((row) => [row.typname, row.values])), expectedEnums);

    const nativeTypes = await client.query(`SELECT table_name, column_name, data_type, udt_name FROM information_schema.columns WHERE table_schema = 'public' AND (table_name, column_name) IN (('agents', 'guardrails'),('workflows', 'graph'),('workflow_dispatches', 'input'),('workflow_runs', 'graph_snapshot'),('workflow_runs', 'spec'),('workflow_runs', 'total_cost'),('tickets', 'run_id'),('messages', 'payload'),('messages', 'sequence_number'),('messages', 'token_usage'),('cost_events', 'computed_cost'),('cost_events', 'cache_read_tokens'),('cost_events', 'cache_write_tokens')) ORDER BY table_name, column_name`);
    assert.deepEqual(nativeTypes.rows.map((row) => [row.table_name, row.column_name, row.udt_name]), [
      ["agents","guardrails","jsonb"],["cost_events","cache_read_tokens","int8"],["cost_events","cache_write_tokens","int8"],
      ["cost_events","computed_cost","numeric"],["messages","payload","jsonb"],["messages","sequence_number","int8"],
      ["messages","token_usage","jsonb"],["tickets","run_id","int8"],["workflow_dispatches","input","jsonb"],
      ["workflow_runs","graph_snapshot","jsonb"],["workflow_runs","spec","jsonb"],["workflow_runs","total_cost","numeric"],
      ["workflows","graph","jsonb"],
    ]);

    const nullableUsage = await client.query(`SELECT column_name, is_nullable FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'cost_events' AND column_name IN ('tokens_in','tokens_out','computed_cost','cache_read_tokens','cache_write_tokens') ORDER BY column_name`);
    assert.deepEqual(nullableUsage.rows.map((row) => [row.column_name, row.is_nullable]), [
      ["cache_read_tokens","YES"],["cache_write_tokens","YES"],["computed_cost","YES"],["tokens_in","YES"],["tokens_out","YES"],
    ]);

    const constraints = await client.query(`SELECT conname FROM pg_constraint c JOIN pg_namespace n ON n.oid = c.connamespace WHERE n.nspname = 'public'`);
    ...
    const foreignKeys = await client.query(`SELECT conname, pg_get_constraintdef(c.oid) AS definition FROM pg_constraint c JOIN pg_namespace n ON n.oid = c.connamespace WHERE n.nspname = 'public' AND c.contype = 'f'`);
    const definitions = Object.fromEntries(foreignKeys.rows.map((row) => [row.conname, row.definition]));
    const expectedForeignKeys = {
      ...
      openclaw_dispatch_inputs_dispatch_id_fkey: "REFERENCES workflow_dispatches(id) ON DELETE RESTRICT",
      ...
      dependencies_blocker_ticket_fk: "REFERENCES tickets(id, project_id) ON DELETE CASCADE",
      ...
      message_consumer_runs_run_id_fkey: "REFERENCES workflow_runs(id) ON DELETE CASCADE",
      message_enqueues_message_id_fkey: "REFERENCES messages(id) ON DELETE RESTRICT",
      ...
      channel_intakes_run_id_fkey: "REFERENCES workflow_runs(id) ON DELETE RESTRICT",
      channel_intakes_workflow_id_fkey: "REFERENCES workflows(id) ON DELETE RESTRICT",
      channel_intakes_last_inbound_message_id_fkey: "REFERENCES messages(id) ON DELETE RESTRICT",
      channel_completion_events_run_id_fkey: "REFERENCES workflow_runs(id) ON DELETE RESTRICT",
      channel_completion_events_completion_message_id_fkey: "REFERENCES messages(id) ON DELETE RESTRICT",
      channel_completion_events_final_outbound_message_id_fkey: "REFERENCES messages(id) ON DELETE RESTRICT",
      ...
      workflow_dispatches_source_message_id_fkey: "REFERENCES messages(id) ON DELETE RESTRICT",
      workflow_dispatches_ticket_id_fkey: "REFERENCES tickets(id) ON DELETE RESTRICT",
      workflow_fanout_groups_agent_id_fkey: "REFERENCES agents(id) ON DELETE RESTRICT",
      workflow_fanout_groups_run_id_fkey: "REFERENCES workflow_runs(id) ON DELETE RESTRICT",
    ...
    assert.deepEqual(Object.keys(definitions).sort(), Object.keys(expectedForeignKeys).sort());
    ...
    assertPlatformToolInvocationRunIndex(await readPlatformToolInvocationIndexKeys(client));

    // Expression-first negative mutation for run_id index
    await client.query("DROP INDEX idx_agent_tool_invocations_run_id");
    try {
      await client.query(`CREATE INDEX idx_agent_tool_invocations_run_id ON agent_tool_invocations ((lower(idempotency_key)), run_id)`);
      ...
      assert.throws(() => assertPlatformToolInvocationRunIndex(expressionFirstKeys), assert.AssertionError);
    } finally {
      await client.query("DROP INDEX IF EXISTS idx_agent_tool_invocations_run_id");
      await client.query(`CREATE INDEX idx_agent_tool_invocations_run_id ON agent_tool_invocations(run_id)`);
    }
    assertPlatformToolInvocationRunIndex(await readPlatformToolInvocationIndexKeys(client));

    const triggers = await client.query(`SELECT tgname FROM pg_trigger WHERE NOT tgisinternal ORDER BY tgname`);
    assert.deepEqual(triggers.rows.map((row) => row.tgname), [
      "agents_90_notify_state_stream","cost_events_90_notify_state_stream",
      "messages_05_preserve_order","messages_10_enforce_ticket_run",
  ...
  await run("preserves retained board, label, blocker, and frontier query shapes", async () => {
    ...
    const nullableExtension = await client.query(`SELECT run_id, acceptance_criteria, assignee_agent_id FROM tickets WHERE identifier = 'FACT-5'`);
    assert.deepEqual(nullableExtension.rows[0], { run_id: null, acceptance_criteria: null, assignee_agent_id: null });

    ids = { project: project.rows[0].id, agent: agent.rows[0].id, workflow: workflow.rows[0].id, run: run.rows[0].id, otherRun: otherRun.rows[0].id, ticket: ticketIds["FACT-2"], nullRunTicket: ticketIds["FACT-5"] };
  });

  await run("enforces message ticket ownership while allowing null-run tickets", async () => {
    await rejectWithCode(client, `INSERT INTO messages (run_id, ticket_id, sender, recipient, type, payload) VALUES ($1, $2, 'system', 'agent:other', 'system', '{}')`, [ids.otherRun, ids.ticket], "23514");
    const retainedTicketMessage = await client.query(`INSERT INTO messages (run_id, ticket_id, sender, recipient, type, payload) VALUES ($1, $2, 'system', 'agent:other', 'system', '{"retained_ticket": true}') RETURNING ticket_id, sequence_number`, [ids.otherRun, ids.nullRunTicket]);
    assert.deepEqual(retainedTicketMessage.rows[0], { ticket_id: ids.nullRunTicket, sequence_number: "1" });
    await rejectWithCode(client, "UPDATE tickets SET run_id = $1 WHERE id = $2", [ids.run, ids.nullRunTicket], "23514");
  });

  await run("reconstructs every message field in deterministic run order", async () => {
    const types = expectedEnums.message_type;
    const expectedTrail = [];
    for (const [index, type] of types.entries()) {
      const item = { ticket_id: index % 2 === 0 ? ids.ticket : null, sender: index % 3 === 0 ? "human:adam" : `agent:${index}`, recipient: index % 2 === 0 ? "agent:orchestrator" : "human:adam", type, payload: { index, artifact: `artifact-${index}` }, handoff_brief: index === 0 ? "Preserve the migration contract." : null, token_usage: index % 2 === 0 ? { input: index + 1, output: index + 2, total: index * 2 + 3 } : null };
      expectedTrail.push(item);
      await client.query(`INSERT INTO messages (run_id, ticket_id, sender, recipient, type, payload, handoff_brief, token_usage) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [ids.run, item.ticket_id, item.sender, item.recipient, item.type, item.payload, item.handoff_brief, item.token_usage]);
    }
    const trail = await client.query(`SELECT sequence_number, ticket_id, sender, recipient, type, payload, handoff_brief, token_usage FROM messages WHERE run_id = $1 ORDER BY sequence_number`, [ids.run]);
    assert.deepEqual(trail.rows.map(({ sequence_number: sequenceNumber, ...message }, index) => { assert.equal(sequenceNumber, String(index + 1)); return message; }), expectedTrail);
    await rejectWithCode(client, `UPDATE messages SET sequence_number = sequence_number + 1 WHERE run_id = $1 AND sequence_number = 1`, [ids.run], "23514");
  });

  await run("does not let a consumer cursor skip an earlier late commit", async () => {
    const runRes = await client.query(`INSERT INTO workflow_runs (workflow_id, status, trigger_type, spec) VALUES ($1, 'running', 'ui', '{"task": "Concurrent append proof"}') RETURNING id`, [ids.workflow]);
    ...
    try {
      ...
      await laterProducer.query("SET LOCAL application_name = 'fact6-later-message-producer'");
      const earlierInsert = await earlierProducer.query(`INSERT INTO messages (run_id, sender, recipient, type, payload) VALUES ($1, 'agent:early', 'agent:next', 'output', '{"order": "early"}') RETURNING sequence_number`, [runId]);
      assert.equal(earlierInsert.rows[0].sequence_number, "1");
      laterInsert = laterProducer.query(`INSERT INTO messages (run_id, sender, recipient, type, payload) VALUES ($1, 'agent:later', 'agent:next', 'output', '{"order": "later"}') RETURNING sequence_number`, [runId]);
      await waitForAdvisoryWait(client, "fact6-later-message-producer");
      const beforeEarlierCommit = await client.query(`SELECT sequence_number FROM messages WHERE run_id = $1 AND sequence_number > 0 ORDER BY sequence_number`, [runId]);
      ...
      const afterEarlierCommit = await client.query(`SELECT sequence_number, payload FROM messages WHERE run_id = $1 AND sequence_number > 0 ORDER BY sequence_number`, [runId]);
      assert.deepEqual(afterEarlierCommit.rows, [{ sequence_number: "1", payload: { order: "early" } }]);
      await laterProducer.query("COMMIT");
      laterTransaction = false;
      const afterLaterCommit = await client.query(`SELECT sequence_number, payload FROM messages WHERE run_id = $1 AND sequence_number > $2 ORDER BY sequence_number`, [runId, 1]);
      assert.deepEqual(afterLaterCommit.rows, [{ sequence_number: "2", payload: { order: "later" } }]);
      const completeTrail = await client.query(`SELECT sequence_number FROM messages WHERE run_id = $1 ORDER BY sequence_number`, [runId]);
      assert.deepEqual(completeTrail.rows.map((row) => row.sequence_number), ["1", "2"]);
    } finally {
      if (earlierTransaction) await earlierProducer.query("ROLLBACK");
      if (laterInsert) { try { await laterInsert; } catch { /* ROLLBACK below is authoritative */ } }
  ...
  await run("rejects invalid, orphaned, negative, and destructive writes", async () => {
    await rejectWithCode(client, `INSERT INTO messages (run_id, sender, recipient, type, payload) VALUES ($1, 'system', 'agent:1', 'bogus', '{}')`, [ids.run], "22P02");
    await rejectWithCode(client, `INSERT INTO messages (run_id, sender, recipient, type, payload) VALUES (999999999, 'system', 'agent:1', 'system', '{}')`, [], "23503");
    await rejectWithCode(client, `INSERT INTO messages (run_id, sender, recipient, type, payload, token_usage) VALUES ($1, 'system', 'agent:1', 'system', '{}', '{"input": -1}')`, [ids.run], "23514");
    await rejectWithCode(client, `INSERT INTO cost_events (run_id, agent_id, model, tokens_in, tokens_out, computed_cost) VALUES ($1, $2, 'test/model', -1, 2, 0.01)`, [ids.run, ids.agent], "23514");
    await rejectWithCode(client, `INSERT INTO cost_events (run_id, agent_id, model, tokens_in, tokens_out, computed_cost, cache_read_tokens, cache_write_tokens) VALUES ($1, $2, 'test/model', 1, 2, 0.01, -1, 0)`, [ids.run, ids.agent], "23514");
    await rejectWithCode(client, `INSERT INTO cost_events (run_id, agent_id, model, tokens_in, tokens_out, computed_cost, cache_read_tokens, cache_write_tokens) VALUES ($1, $2, 'test/model', 1, 2, -0.01, 0, 0)`, [ids.run, ids.agent], "23514");
    await rejectWithCode(client, `INSERT INTO dependencies (project_id, blocker_ticket_id, blocked_ticket_id) VALUES ($1, $2, $2)`, [ids.project, ids.ticket], "23514");
    await client.query(`INSERT INTO cost_events (run_id, agent_id, model, tokens_in, tokens_out, computed_cost, cache_read_tokens, cache_write_tokens) VALUES ($1, $2, 'test/model', 12, 8, 0.0042, 20, 3)`, [ids.run, ids.agent]);
    await client.query(`INSERT INTO cost_events (run_id, agent_id, model, tokens_in, tokens_out, computed_cost, cache_read_tokens, cache_write_tokens) VALUES ($1, $2, 'test/model', NULL, 0, NULL, NULL, 0)`, [ids.run, ids.agent]);
    await rejectWithCode(client, "DELETE FROM workflows WHERE id = $1", [ids.workflow], "23503");
    await rejectWithCode(client, "DELETE FROM workflow_runs WHERE id = $1", [ids.run], "23503");
    await rejectWithCode(client, "DELETE FROM agents WHERE id = $1", [ids.agent], "23503");
  });

  await client.end();

  process.stderr.write(`\n1..${passed + failed}\n`);
  process.stderr.write(`# tests ${passed + failed}\n# pass ${passed}\n# fail ${failed}\n`);
  process.exit(failed > 0 ? 1 : 0);
} catch (err) {
  console.error(`Bail out! ${err.message}`);
  try { await client.end(); } catch { /* expected */ }
  process.exit(1);
}
